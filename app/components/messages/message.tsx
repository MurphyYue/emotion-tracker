        ...
        <div className="space-y-3">
          {message.role === "assistant" ? (
            <div className="flex items-center space-x-4">
              <OpenAISVG
                className="border-primary bg-primary text-secondary rounded border-DEFAULT p-1 size-8"
              />
          ...
          <div className={cn("flex", message.role === "assistant" ? "justify-start" : "justify-end")}>{message.content}</div>
        </div>
      </div>
    </div>
  );
